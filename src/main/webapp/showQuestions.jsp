...
try {
    ...
    try (Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/projectdb","root","school")) {
        ...
        String fetchQ = "SELECT q.question_id, q.question_text, q.asked_at, q.answer, q.handled_by, eu.username AS asker "
                      + "FROM Question q "
                      + "JOIN eu_account eu ON q.username = eu.username "
                      + "ORDER BY q.asked_at DESC";

        try (PreparedStatement ps = con.prepareStatement(fetchQ);
